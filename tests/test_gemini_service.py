...
def test_gemini_service_init_success(monkeypatch):
    ...
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.get_config", lambda: {"GOOGLE_GEMINI_API_KEY": "key123"})
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.genai.configure", lambda api_key: None)
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.logger", MagicMock())
...
def test_gemini_service_init_no_api_key(monkeypatch):
    ...
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.get_config", lambda: {})
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.genai.configure", lambda api_key: None)
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.logger", MagicMock())
...
def test_gemini_service_init_with_instructions(monkeypatch):
    ...
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.genai.configure", lambda api_key: None)
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.logger", MagicMock())
    monkeypatch.setattr(GeminiService, "load_system_instructions_from_json", lambda self, path: "INST")
...
def test_gemini_service_get_response(monkeypatch):
    ...
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.genai.GenerativeModel", lambda name: mock_model)
    monkeypatch.setattr("src.infrastructure.google_generative_ai.gemini_service.logger", MagicMock())
    service = GeminiService()
